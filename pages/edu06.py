...
with tab2:
    ...
    """
    <div class='info-box info-box-pink'>
        <p>권한 상승1: 에이전트에게 부여된 과도한 자율성으로 인해, 권한 검증 절차 없이 LLM이 사용자의 입력을 그대로 신뢰해 관리자 권한을 부여할 수 있으며, 이를 통해 시스템의 주요 기능에 접근 가능</p>
        <p>권한 상승2: 특히 OCR 기능이 확장된 LLM이 텍스트와 이미지를 별도로 처리할 경우, 이미지 내에 삽입된 악성 프롬프트를 적절히 검증하지 못하면 기존의 권한 검증 절차가 존재하더라도 이를 우회하고 관리자 권한을 잘못 부여하는 과도한 위임이 발생</p>
        <p>이메일 도용: 사용자가 자신의 신원을 관리자로 위장해 이메일 전송을 요청하고, LLM이 이를 수행함으로써 관리자 계정의 악용 가능성 존재</p>
        <p>정보 유출: 최고 관리자 권한을 획득한 공격자는 DB 조회 권한을 악용하여 플래그와 같은 내부 민감 정보를 조회</p>
        <p>기능 오남용 및 시스템 무결성 손상: 관리자 권한을 악용하여 에이전트에 부여된 과도한 기능을 통해 데이터 변조, 시스템 설정 변경 등 시스템의 신뢰성과 무결성을 훼손</p>
    </div>
    """,
    unsafe_allow_html=True
)
