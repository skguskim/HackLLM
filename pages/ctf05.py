...
st.markdown(
    ...
    <div style="background-color: #f9f9f9; padding: 20px; border-radius: 12px; border-left: 6px solid #f39c12; margin-top: 30px; margin-bottom: 40px;">
        ...
        웹 애플리케이션에서 사용자 입력을 제대로 검증하지 않아 <strong>악성 스크립트가 실행</strong>될 수 있는 취약점입니다.<br>
        공격자는 다른 사용자의 브라우저에서 <strong>JavaScript 코드를 실행</strong>할 수 있습니다.</p>

        <p><strong>XSS의 주요 유형:</strong></p>
        <ul>
            <li><strong>Reflected XSS</strong>: 사용자 입력이 즉시 페이지에 반영되어 실행됩니다.</li>
...
따라서 XSS 취약점을 활용하여 `admin_session` 쿠키를 탈취해 보시기 바랍니다. 🔥
