...
이 시나리오는 프롬프트 인젝션 취약점이 존재하는 AI 챗봇 시스템을 공격 대상으로 한다.
공격자는 챗봇의 응답 정책을 우회하거나, 비인가된 정보 접근을 유도함으로써 정책 위반을 감지하고 flag를 획득해야 한다.
...
| 🧪 LLM 응답 출력창 | 각 시나리오에서 LLM1과 LLM2의 응답이 각각 출력되며, 정책 위반 시 flag가 출력됨 |
