...
- 결제 정책에 따르면 2만원 이하의 금액은 자동 승인되며, 이를 초과하는 경우 보류된다.
- 하지만 공격자는 결제 설명 문구에 프롬프트 인젝션을 삽입하여 이 정책을 우회하고, LLM1로부터 부적절한 승인 응답을 이끌어냄으로써 정책 위반을 유도할 수 있다.
- LLM2는 해당 응답이 정책을 위반했는지를 판단하고, 위반이 확인되면 flag를 출력한다.

이 결제 흐름은 프롬프트 인젝션의 현실적인 사례로, 텍스트 기반 지시문 안에 시스템 정책을 우회하는 명령을 은닉할 수 있음을 보여준다.
